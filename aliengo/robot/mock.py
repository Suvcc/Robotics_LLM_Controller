...
class MockRobotController:
    ...
    def set_visible_objects(self, objects: list[dict]) -> None:
        with self._lock:
            self.visible_objects = deepcopy(objects)
    ...
    def stand_up(self) -> SkillResult:
        with self._lock:
            err = self._estop_error("stand_up")
            if err:
    ...
    def sit_down(self) -> SkillResult:
        with self._lock:
            ...
            err = self._precheck("sit_down", None, COST_SIT, allow_during_estop=True)
            if err:
    ...
    def _precheck(
        ...
        *,
        ...
        if injected:
            return injected
        if self._estopped and not allow_during_estop:
            return f"Cannot {action}: emergency stop is active."
    ...
    def _estop_error(self, action: str) -> str | None:
        if self._estopped:
            return f"Cannot {action}: emergency stop is active."
        return None
